...
public class LoginView1 extends JFrame implements ActionListener {
	// 생성자
	...
	public static void main(String[] args) {
		LoginView loginView = new LoginView(); // new LoginView(); 이렇게 써도 됨!
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(obj == jbtn_login) {
			...
			// 오라클 서버에서 반환 값 담기 
			String result = ""; // 3개중 하나 // 이름(1) or 0(비번이 틀림) or -1(아이디가 존재하지 않음)
			LoginDao loginDao = new LoginDao(); // 인스턴스화가 되더라도 이제 더 이상 initDisplay가 호출 안됨 .왜? 로그인다오에서의 메인에 이동해서
