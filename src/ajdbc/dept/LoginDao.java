...
public class LoginDao extends JFrame implements ActionListener {
	...
	public LoginDao() { // 생성자호출
		jbtn_check.addActionListener(this); // 콜백리스너
		// 화면을 그리는 메소드 호출
		// initDisplay();
	}
	public void initDisplay() {
		jp_north.setLayout(new BorderLayout()); // 동서남북으로 배치
		jp_north.add("Center", jtf_id);
		jp_north.add("East", jbtn_check);
		this.add("North", jp_north); // 아이디중복검사했을때 활성화되겠끔 만들어버리자.
	...
	public String login(String mem_id, String mem_pw) {
        ...
        sql.append("             CASE                                  ");
        sql.append("                 WHEN mem_pw =? THEN mem_name    ");
        sql.append("                 ELSE '0'                          ");
        sql.append("             END                                   ");
        sql.append("             ELSE '-1'                             ");
		...
		try {
			...
			// 53번 ? 자리에 들어갈 아이디를 설정해야 함
			pstmt = con.prepareStatement(sql.toString());
			pstmt.setString(1, mem_id);
			pstmt.setString(2, mem_pw);
			// select 처리시는 executeQuery() 호출
	...
	public void actionPerformed(ActionEvent e) {
		...
		// ID 중복체크버튼 눌린거야?
		if (obj == jbtn_check) {
			System.err.println("ID중복체크 호출 성공");
			...
			// 입력한 아이디가 없는데?
				...
				// 입력한 아이디를 사용할 수 있어
				JOptionPane.showMessageDialog(this, "입력한 아이디는 사용할 수 있습니다.", "INFO", JOptionPane.INFORMATION_MESSAGE);
