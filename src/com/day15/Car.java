...
public abstract class Car implements MoveBehavior{
	...
	// 추상메소드는 어떤 자동차가 될지도 정할 수가 없어서 추상메소드로 하기로 함
	// 세미콜론으로 끝나는 메소드는 추상메소드 이다.
	public abstract void display();
	// 좌중괄호와 우중괄호가 있는 메소드를 일반 메소드라 한다.
	@Override
	public void run() {// 엑셀을 밟으면 그때 마다 호출된다.
		speed = speed + 1;
	}
}
 ...
 * 추상클래스와 인터페이스 때문에 선언부의 타입봐 생성부의 타입이 다를 수 있다.
 * 단 자바는 이럴 경우 생성부의 타입으로 객체가 생성된다. 메모리에 로딩 된다.
 * 
 * 클래스 타입 변수명 = new 클래스 타입() ; - 스탠다드
 * 추상클래스 변수명 = new 구현체클래스 () ; - 잘하는 사람 - 다형성 가능해짐 - 코드수정이 줄어듬
