...
public class RetrieveAddrEty {// 조회(전체조회, 상세조회)
	...
	public AddressVO[] retrieve() { // 전체조회
		...
		sql.append("SELECT id, name, address, gender,telephone   ");
		...
		try {
			...
			addressVO = new ArrayList<>();
			Map<Object, String> mvo = null;

			int i = 0;

			while (rs.next()) {
				//rvo = new Address(rs.getString("name"); 이런식으로도 작성가능
				int id = rs.getInt("id");
	...
	public AddressVO[] retrieve(AddressVO vo) { // 상세조회
		...
		sql.append(" WHERE  id = ? ");
		AddressVO[] aArr = new AddressVO[1];
		AddressVO aVO = null;
		...
		try {
			...
			aVO = new AddressVO(id, name, address, telephone,gender,relationship,birthday,comments,registedate);
			aArr[i++] = vo ;
