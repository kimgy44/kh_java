...
public class AddressBook extends JFrame {
	...
	private void addActionPerformed(ActionEvent evt) {
		...
		// 호출할 때 파라미터로 사용자가 선택한 버튼의 이름, 화면에 보여줄까?, AddressVO(왜냐면 새로 입력하는거니까)
		// null, abook의 주소번지(원본-공유-static) - 이태릭체 - static타입이라서
		// 1) 하나의 화면으로 입력 or 수정 or 상세조회
		// 2) 입력, 수정 : 각 입력 컴포넌트가 수정할 수 있는 상태
		//   상세조회 : onlyread - false 
		// 2) 입력 -> view -> action(insert) -> action(selectall) -> view
...
//				Integer id= (Integer)myTableModel.getValueAt(index[0], 0);
	...
	private void deleteActionPerformed() {
		...
		if(index.length == 0) {
			...
			try {
				...
				mDialog.set("삭제", true, newVo, abook);
				mDialog.setVisible(true);
			} catch (Exception e) {
				JOptionPane.showMessageDialog(this, "데이터를 가져오는 중 발생했습니다."+e.toString(),"Error", JOptionPane.ERROR_MESSAGE);
			}
	...
	public void refreshData() {
		...
		// 전체조회 감지되면 이벤트처리 - refreshData()호출
		
		// 전체조회인경우 이 아래 두가지 없애도 될
		//AddressVO vo = new AddressVO();
		...
		if(vos == null || vos.length == 0) {
			/*
			 * vos = new AddressVO[2]; AddressVO rvo = new
			 * AddressVO("이순신","서울시 마포구 공덕동","010-555-6677","1"
			 * ,"고교동창","1990-05-28","Back-End개발자","2022-03-15",1); vos[0] = rvo; rvo = new
			 * AddressVO("강감찬","서울시 영등포구 당산동","010-777-6677","1"
			 * ,"대학동창","1992-01-28","Back-End개발자","2022-01-25",2); vos[1] = rvo;
			 */
