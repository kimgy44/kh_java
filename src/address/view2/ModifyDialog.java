...
class ModifyDialog extends JDialog {
  	...
  	private void btnOkayActionPerformed(ActionEvent evt) {
		 ...
		 */
			...
			try {
				...
				vo.setBirthday(getBirthDay());
				vo.setComments(getComment());
				/*  vo를 뒤에 다 두면 왜 안되는걸까?!!!!!!!!!!!!!!!!!!!vo에서 가져오기에
				 * 
				 * 
				 * setName(vo.getName()); setAddress(vo.getAddress());
				 * setTel(vo.getTelephone()); setGender(vo.getGender());
				 * setRelationShip(vo.getRelationship()); setBirthDay(vo.getBirthday());
				 * setComment(vo.getComments()); setRegDate(vo.getRegistedate());
				 */
	...
	public void setValue(AddressVO vo) {
		...
		// 여기서부터 !!!!!!!!!!!!!!!!
		/*
		 * int num = sc.next(); String name = sc.next();
		 */
